...
/*!50001 VIEW `view_user_locations` AS select `u`.`iduser` AS `iduser`,`u`.`user_name` AS `user_name`,`u`.`phone_number` AS `phone_number`,`u`.`created_at` AS `created_at`,`u`.`updated_at` AS `updated_at`,`l`.`idlandmark` AS `idlandmark`,`l`.`tag` AS `tag`,`l`.`type_id` AS `type_id`,`l`.`name` AS `name`,`l`.`description` AS `description`,`l`.`x` AS `x`,`l`.`y` AS `y`,`l`.`user_id` AS `user_id`,`l`.`style` AS `style`,`l`.`img` AS `img`,concat(ifnull(`u`.`iduser`,''),'_',ifnull(`l`.`idlandmark`,'')) AS `iduser_idlandmark` from (`user` `u` left join `landmark` `l` on((`u`.`iduser` = `l`.`user_id`))) */;
