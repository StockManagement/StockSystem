...
public class sidePanelLandmarkControl {
    ...
    public void save() {
        ...
        if (landmarkPositionX == null || landmarkPositionX.length() == 0 || landmarkPositionY == null || landmarkPositionY.length() == 0) {
            return;
        }

        if (selectedSymbol == "") {
            selectedSymbol = "resources/img/client/user-home.png";
        } else {
            String defultpath = Helper.getConfig("DefaultServerPath");
            selectedSymbol = selectedSymbol.replace(defultpath, "");
